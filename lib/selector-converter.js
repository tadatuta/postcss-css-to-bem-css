...
module.exports = function (cssSelector, opts) {
  ...
  return selectorParser(selectors => {
    selectors.walkClasses(selector => {
      // ugly hack
      // not BEM transformations, prefix only
      if (
        !opts.sourceNaming &&
        !opts.targetNaming &&
        opts.transforms && opts.transforms.prefix && Object.keys(opts.transforms).length === 1
      ) {
        selector.replaceWith(
          selectorParser.pseudo({ value: '.' + opts.transforms.prefix + selector._value })
        )
        return;
      }
